 ...
 * The input can be a single email string or an array of email strings.
 * @param mixed $emails The input emails.
 * @param string $field_name The name of the field for error messages.
 * @return array An array of validated email addresses.
 */
function validate_and_normalize_emails($emails, $field_name) {
    if (empty($emails)) {
        return [];
    }

    $validated_emails = [];
    $email_list = is_array($emails) ? $emails : [$emails];

    foreach ($email_list as $email) {
        $trimmed_email = trim($email);
        if (!filter_var($trimmed_email, FILTER_VALIDATE_EMAIL)) {
            send_json_error(400, "Invalid email address in `{$field_name}`: " . htmlspecialchars($email));
        }
        $validated_emails[] = $trimmed_email;
    }
...
$to_emails = validate_and_normalize_emails($json_data['to_emails'] ?? null, 'to_emails');
if (empty($to_emails)) {
    send_json_error(400, 'At least one valid recipient email must be provided in `to_emails`.');
}

$cc_emails = validate_and_normalize_emails($json_data['cc_emails'] ?? null, 'cc_emails');
$bcc_emails = validate_and_normalize_emails($json_data['bcc_emails'] ?? null, 'bcc_emails');
...
try {
    ...
    $stmt->execute([
        ...
        $profile_id,
        $client_ip,
        json_encode($to_emails),
        empty($cc_emails) ? null : json_encode($cc_emails),
